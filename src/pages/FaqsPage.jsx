...
export default function FaqsPage() {
  ...
  async function submit(e) {
    ...
    try {
      ...
      setModalOpen(false)
      setForm({ id: null, order: 1, question: '', answer: '' })
      setToast({ message: isEditing ? 'FAQ updated!' : 'FAQ created!', type: 'success' })
      await load()
    } catch (err) {
      if (String(err.message).includes('Unauthorized')) {
        navigate('/login', { replace: true })
  ...
  return (
    <>
      ...
      />
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={isEditing ? `Edit FAQ #${form.id}` : 'Create FAQ'}>
